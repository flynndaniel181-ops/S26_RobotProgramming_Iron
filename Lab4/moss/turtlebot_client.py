...
from turtle_interfaces.srv import SetColor, SetPenSize  #imported new service SetPenSize
...
class TurtleClient(Node):
    def __init__(self):
        ...
        #### parameters ####
        ## Turtle Color ##
        self.declare_parameter('turtleColor', 'green', ParameterDescriptor(description='color of the turtle on startup'))
        ...
        ## Turtle Pen Size##
        #declare pen size parameter with default value of 3
        self.declare_parameter('turtlePenSize', 3, ParameterDescriptor(description='size of the pen on startup'))
        #get the pen size parameter value
        turtlePenSize = self.get_parameter('turtlePenSize').get_parameter_value().integer_value
        #apply pen size to turtle display
        self.turtle_display.pensize(turtlePenSize)
        #create client for pen size service to update pen size on server
        self.pen_size_cli = self.create_client(SetPenSize, 'set_Pen_Size')
        #wait for service available
        while not self.pen_size_cli.wait_for_service(timeout_sec=1.0):
            self.get_logger().info('Pen Size service not available, waiting...')
        #create the service request and set to value
        self.pen_size_req = SetPenSize.Request()
        self.pen_size_req.pen_size = turtlePenSize
        #send async request to server so it uses the correct pen size
        self.server_call = True
        self.service_future = self.pen_size_cli.call_async(self.pen_size_req)
    ...
    def update(self):
        ...
        if self.turtle.color == 'None':
            ...
            self.turtle_display.pensize(self.turtle.pen_size)  #added to actually display the updated pen size
